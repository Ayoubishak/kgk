...
namespace Biblio
{
    public partial class Provenance : Form
    {
        ...
        private void reposSup_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                var ID = gridView_Provena.GetRowCellValue(gridView_Provena.FocusedRowHandle, "Prov_Id").ToString();

                if (ID != "" || ID != null)
                {

                    SqlCommand Del = new SqlCommand();
                    Del.CommandText = "delete [dbo].[P_Provenance]  where [Prov_Id]='" + Int32.Parse(ID) + "'";
                    Del.Connection = DAL.sqlconnection;
                    Del.ExecuteNonQuery();

                    textBox_CodeProvenance.Text = "";
                    textBox_Inti.Text = "";
                ...
                gridControl_Provena.DataSource = ds;
                gridControl_Provena.DataMember = "table";
                gridView_Provena.BestFitColumns();
            }
            catch (Exception ex)
            {
        ...
        private void repositConsulter_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            try
            {
                ...
                textBox_CodeProvenance.Text = gridView_Provena.GetRowCellValue(gridView_Provena.FocusedRowHandle, "Prov_Id").ToString();
                textBox_Inti.Text = gridView_Provena.GetRowCellValue(gridView_Provena.FocusedRowHandle, "Prov_Intitule").ToString();
                var Statu = gridView_Provena.GetRowCellValue(gridView_Provena.FocusedRowHandle, "Prov_Statut").ToString();
        ...
        private void windowsUIButtonPanel1_ButtonClick(object sender, DevExpress.XtraBars.Docking2010.ButtonEventArgs e)
        {
            if (e.Button == windowsUIButtonPanel1.Buttons[0])
            {
                ...
                string sql3 = "select [Prov_Intitule] from [dbo].[P_Provenance] where [Prov_Intitule]='" + textBox_Inti.Text.Trim() + "'";
                //SqlConnection connection = new SqlConnection(connectionString);
                ...
                cmdAjouterB.CommandText = "INSERT INTO [dbo].[P_Provenance] ([Prov_Intitule],[Prov_Statut]) VALUES  (@Intitule,@Statut)";
